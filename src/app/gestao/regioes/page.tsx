...
export default function RegioesPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // Restaura seleção salva — se não houver, usa o primeiro estado disponível
      ...
      const estadoEfetivo = estadoSalvo || Array.from(estadosMap.keys())[0] || ''
      if (!estadoEfetivo) return

      setEstadoSel(estadoEfetivo)
      const cidadesFiltradas = listaCidadesAll.filter((c) => c.estadoId === estadoEfetivo)
      setCidades(cidadesFiltradas)

      const cidadeEfetiva = cidadeIdSalvo || cidadesFiltradas[0]?.id || null
      if (!cidadeEfetiva) return
      const cidade = cidadesFiltradas.find((c) => c.id === cidadeEfetiva)
      if (!cidade) return
